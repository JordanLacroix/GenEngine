...
La progression est **agrégée**, jamais recalculée : `ScenarioMastery` reste la source de vérité. Pour un parcours comme pour une catégorie, le service expose le nombre de scénarios, le nombre entamés, le nombre terminés et un pourcentage. Un scénario est entamé dès qu'une maîtrise existe, terminé dès qu'une fin a été atteinte, et le pourcentage est la moyenne des maîtrises sur tous les scénarios de la portée, les scénarios jamais joués comptant pour zéro. Un scénario joué sur plusieurs versions publiées est retenu par sa meilleure maîtrise.
