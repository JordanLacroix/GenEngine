...
- `GET /asset-packs/{packId}/files/{chemin}` — octets d'un asset, en `image/svg+xml`, `image/png`, `audio/ogg`, `audio/mp4` (`.m4a`), `audio/aac` (`.aac`) ou `audio/mpeg` (`.mp3`), avec `Cache-Control: public, max-age=31536000, immutable` et `X-Content-Type-Options: nosniff`. La liste est une **liste blanche** : toute autre extension renvoie 404, jamais un type deviné.
