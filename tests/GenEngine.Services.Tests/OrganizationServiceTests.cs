...
public sealed class OrganizationServiceTests
{
    ...
    public async Task PlayerContextOnlyContainsEffectiveAssignmentsFromTheRequestedFront()
    {
        ...
        _ = await service.UpsertMembershipAsync("default", Guid.NewGuid(), defaultUnit, userId, MembershipKind.Participant, Now.AddDays(-1), null, true, null, default);
    ...
    public async Task ExpiredMembershipRemovesCatalogAccess()
    {
        ...
        _ = await service.UpsertMembershipAsync("default", Guid.NewGuid(), unitId, userId, MembershipKind.Participant, Now.AddDays(-2), Now.AddDays(-1), true, null, default);
