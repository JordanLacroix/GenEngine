...
public sealed record MembershipView(Guid Id, string FrontId, Guid UnitId, Guid UserId, MembershipKind Kind, DateTimeOffset StartsAt, DateTimeOffset? EndsAt, bool IsActive, int Revision, DateTimeOffset UpdatedAt);
...
public sealed record PlayerOrganizationContextView(string FrontId, bool IsMember, IReadOnlyList<Guid> UnitIds, IReadOnlyList<Guid> SupervisedUnitIds, IReadOnlyList<AssignmentView> Assignments);
...
public sealed class OrganizationService(IOrganizationRepository repository, TimeProvider timeProvider)
{
    ...
    public async Task<MembershipView> UpsertMembershipAsync(string frontId, Guid id, Guid unitId, Guid userId, MembershipKind kind, DateTimeOffset startsAt, DateTimeOffset? endsAt, bool isActive, int? expectedRevision, CancellationToken cancellationToken)
    {
        _ = await RequireActiveFrontAsync(frontId, cancellationToken).ConfigureAwait(false);
        ...
        if (membership is null)
        {
            ...
            membership = Membership.Create(id, frontId, unitId, userId, kind, startsAt, endsAt, now);
            await repository.AddMembershipAsync(membership, cancellationToken).ConfigureAwait(false);
        }
        else
        {
            if (membership.UnitId != unitId || membership.UserId != userId) throw new OrganizationException("membership_identity_immutable", "Membership unit and user cannot be changed.");
    ...
    private static MembershipView Map(Membership item) => new(item.Id, item.FrontId, item.UnitId, item.UserId, item.Kind, item.StartsAt, item.EndsAt, item.IsActive, item.Revision, item.UpdatedAt);
