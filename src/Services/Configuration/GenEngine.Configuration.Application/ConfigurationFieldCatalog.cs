...
public static class ConfigurationFieldCatalog
{
    ...
    [
        ...
        new("aiProviders[].secretReference", "Référence de secret", "Désigne le secret, ne le contient jamais. Vide signifie « ce fournisseur ne demande aucun identifiant ». Un secret introuvable n'est pas une erreur : le fournisseur est vu comme non configuré et l'appelant se replie sur Offline.", "env:GENENGINE_AI_AZURE_FOUNDRY_KEY", "Grammaire `schéma:identifiant`, 512 caractères au plus, sinon `invalid_secret_reference`. Schéma `env` implémenté (variable `[A-Z_][A-Z0-9_]*`), `vault` réservé. Retirée de la configuration publiée. Voir « Références de secrets » dans platform-configuration.md, qui fait foi."),
        ...
        new("branding.applicationName", "Nom de l'application", "Le nom du produit affiché dans les clients, les onglets et l'écran de connexion. Vide, les clients retombent sur leur propre défaut.", "Le Diapason", "1 à 80 caractères."),
        new("branding.shortName", "Nom court", "La forme abrégée utilisée quand la place manque : icône d'application, onglet étroit, notification.", "Diapason", "1 à 24 caractères."),
        new("branding.tagline", "Accroche", "La phrase de marque affichée sous le nom avant la connexion.", "Une réponse fluide n'est pas une réponse vérifiée.", "1 à 160 caractères."),
        new("branding.brandIconUrl", "Icône de marque", "L'icône représentant la marque à l'intérieur de l'application.", "https://exemple.org/marque.svg", "URL HTTPS absolue ou référence de pack packId:assetId."),
        new("branding.clientIconUrl", "Icône du client", "L'icône installée sur l'appareil pour l'application mobile ou de bureau.", "https://exemple.org/app-icon.png", "URL HTTPS absolue ou référence de pack."),
        new("branding.logoUrl", "Logo", "Le logo complet affiché en tête des écrans publics.", "https://exemple.org/logo.svg", "URL HTTPS absolue ou référence de pack."),
        new("branding.faviconUrl", "Favicon", "L'icône d'onglet du client web.", "https://exemple.org/favicon.png", "URL HTTPS absolue ou référence de pack."),
        new("branding.theme.colors", "Couleurs du thème", "Les jetons de couleur de l'instance, jeton par jeton. Les huit jetons documentés sont obligatoires pour qu'un client n'ait jamais à inventer une valeur de repli ; vous pouvez en ajouter d'autres.", "ink → #17344a", "Doit définir ink, surface, accent, accentAlt, success, warning, danger et muted ; 60 entrées au plus ; chaque valeur en #RRGGBB ou #RRGGBBAA."),
        new("branding.theme.colorScheme", "Schéma de couleurs", "Impose le mode clair ou sombre, ou laisse l'appareil décider.", "Light", "Dark, Light ou Auto."),
        new("branding.theme.cornerRadius", "Rayon des angles", "L'arrondi appliqué aux cartes et aux boutons, en points.", "12", "Entre 0 et 64."),
        new("branding.theme.fontFamily", "Police", "La pile de polices utilisée par les clients. Ils retombent sur leur police système si elle est absente.", "Georgia, \"Times New Roman\", serif", "160 caractères au plus."),
        new("branding.accentPalette", "Palette d'accents", "Associe chaque jeton d'accent nommé — celui des catégories, des parcours et du familier — à une couleur réelle. C'est ce qui rend ces accents effectivement affichables : un jeton non associé laisse une catégorie sans couleur.", "encre → #17344a", "60 entrées au plus ; nom ≤ 40 caractères ; chaque valeur en #RRGGBB ou #RRGGBBAA."),
        ...
        new("finale.conditions[].type", "Type de condition", "Ce que la condition mesure dans la progression déjà enregistrée. Le même vocabulaire sert aux récompenses conditionnelles.", "JourneyCompleted", "ScenariosCompleted, CategoryCompleted, JourneyCompleted, EndingsReached, MasteryPercentReached ou PlayerStatReached."),
        ...
        new("finale.conditions[].statKey", "Statistique visée", "La clé de la statistique joueur dont le seuil est mesuré.", "lucidite", "Requise pour PlayerStatReached ; doit figurer dans playerStats.stats."),
        ...
        new("rewards.enabled", "Récompenses actives", "Désactivé, le catalogue reste publié et les récompenses déjà obtenues restent visibles, mais aucune nouvelle n'est plus estampillée. Couper les récompenses ne retire jamais ce qu'un joueur a gagné.", "true", null),
        new("rewards.rewards[].id", "Identifiant de la récompense", "La clé stable de la récompense. Elle est mémorisée sur le profil au moment où elle est obtenue, donc elle n'est jamais réattribuée.", "c4e19a55-3f27-4d8b-9a61-72d0b5e3f101", "Unique dans le document."),
        new("rewards.rewards[].enabled", "Récompense active", "Une récompense inactive n'est jamais évaluée ni obtenue. Celles déjà obtenues restent affichées.", "true", null),
        new("rewards.rewards[].label", "Libellé", "Le nom affiché de la récompense.", "Cinq fois plutôt qu'une", "Obligatoire, 120 caractères au plus."),
        new("rewards.rewards[].description", "Description", "Ce que le joueur a accompli, écrit pour lui. C'est aussi ce qu'il lit tant qu'il ne l'a pas obtenue.", "Vous avez traversé cinq situations complètes.", "Obligatoire, 500 caractères au plus."),
        new("rewards.rewards[].mode", "Mode de combinaison", "Exiger toutes les conditions, ou n'en exiger qu'une seule.", "All", "All ou Any."),
        new("rewards.rewards[].visualUrl", "Visuel de la récompense", "L'image affichée sur le badge.", "https://exemple.org/haut-fait.avif", "URL HTTPS absolue ou référence de pack."),
        new("rewards.rewards[].labelKey", "Clé de libellé", "La clé du dictionnaire utilisée si vous préférez piloter le libellé par le vocabulaire.", "rewards.cinqScenarios", null),
        new("rewards.rewards[].conditions[].id", "Identifiant de condition", "La clé stable de la condition, utilisée pour afficher sa progression au joueur.", "(guid)", "Unique dans la récompense."),
        new("rewards.rewards[].conditions[].type", "Type de condition", "Ce que la condition mesure. C'est exactement le vocabulaire de la fin de jeu : les deux blocs partagent un seul modèle de conditions.", "ScenariosCompleted", "ScenariosCompleted, CategoryCompleted, JourneyCompleted, EndingsReached, MasteryPercentReached ou PlayerStatReached."),
        new("rewards.rewards[].conditions[].description", "Description de la condition", "Ce que le joueur doit accomplir, écrit pour lui.", "Avoir terminé au moins cinq scénarios.", null),
        new("rewards.rewards[].conditions[].threshold", "Seuil", "Le nombre, le pourcentage ou le nombre de points à atteindre, selon le type.", "5", "Requis pour ScenariosCompleted, MasteryPercentReached et PlayerStatReached."),
        new("rewards.rewards[].conditions[].categoryId", "Catégorie visée", "La catégorie à terminer entièrement.", "(identifiant de catégorie)", "Requis et existant pour CategoryCompleted."),
        new("rewards.rewards[].conditions[].journeyId", "Parcours visé", "Le parcours à terminer entièrement.", "(identifiant de parcours)", "Requis et existant pour JourneyCompleted."),
        new("rewards.rewards[].conditions[].endingIds[]", "Fins visées", "Les identifiants de fins de scénario à avoir atteintes.", "fin-rupture-silence", "Requis et non vide pour EndingsReached."),
        new("rewards.rewards[].conditions[].scenarioIds[]", "Scénarios visés", "Restreint le décompte à ces scénarios. Vide signifie tous les scénarios.", "(identifiants de scénario)", null),
        new("rewards.rewards[].conditions[].statKey", "Statistique visée", "La clé de la statistique joueur dont le seuil est mesuré.", "lucidite", "Requise pour PlayerStatReached ; doit figurer dans playerStats.stats."),
        new("rewards.rewards[].grants[].type", "Nature de l'octroi", "Ce que la récompense donne réellement. Les trois natures ne se comportent pas pareil : un haut fait et un titre sont des marques inertes, une valeur d'économie crédite le portefeuille.", "Achievement", "Achievement, Title ou Currency."),
        new("rewards.rewards[].grants[].label", "Libellé de l'octroi", "Ce que le joueur lit : le nom du haut fait, du titre, ou le motif inscrit dans son portefeuille.", "Celui qui va au bout", "Obligatoire, 120 caractères au plus."),
        new("rewards.rewards[].grants[].reference", "Référence stable", "Le slug sous lequel le haut fait ou le titre est mémorisé et rendu par les clients. Il ne se traduit pas et ne se renomme pas.", "celui-qui-va-au-bout", "Requis pour Achievement et Title, interdit pour Currency ; 1 à 60 caractères parmi a-z, 0-9 et « - »."),
        new("rewards.rewards[].grants[].amount", "Montant crédité", "Les points de monnaie narrative crédités au portefeuille, une seule fois.", "100", "Requis et strictement positif pour Currency, interdit pour les autres natures ; 1 000 000 au plus."),
    ];
}